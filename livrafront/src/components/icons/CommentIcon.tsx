interface CommentIconProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
}

export default function CommentIcon({
  size,
  ...props }: CommentIconProps) {

  return (
    <svg
      width={size}
      height={size}
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 40 40"
      stroke="currentColor"
      {...props}
    >
      <path d="M35 19.1667C35.0057 21.3665 34.4918 23.5365 33.5 25.5C32.3241 27.853 30.5162 29.832 28.2791 31.2155C26.0419 32.599 23.4637 33.3324 20.8333 33.3334C18.6336 33.3391 16.4635 32.8252 14.5 31.8334L5 35L8.16667 25.5C7.17489 23.5365 6.66093 21.3665 6.66667 19.1667C6.66768 16.5363 7.40102 13.9581 8.78453 11.721C10.168 9.4838 12.1471 7.67599 14.5 6.50005C16.4635 5.50827 18.6336 4.99431 20.8333 5.00005H21.6667C25.1406 5.1917 28.4217 6.65798 30.8819 9.11815C33.3421 11.5783 34.8083 14.8595 35 18.3334V19.1667Z" fill="none" strokeWidth="2" />
</svg>

  );
}