...
export default function Comunidade({ id, nome, descricao, image }: ComunidadeCardProps) {
    return (
        <Link href={`/comunidade/${titleToSlug(nome)}`} className="w-full flex flex-col items-center p-4 gap-2 hover:shadow-md transition-shadow medium-border-radius">
            {/*Coluna da Imagem*/}
            <div className="w-3/5 aspect-square relative overflow-hidden rounded-full border-[3px] text-[var(--primary-700)] bg-[#3D552F]">
                {image ? (<Image
                    src={image}
            ...
            {/*Coluna do conteúdo*/}
            <div className="flex flex-col w-full items-center align-middle text-center pl-2">
