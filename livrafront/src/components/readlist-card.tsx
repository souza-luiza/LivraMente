...
export function ReadlistCard({ r, userId }: { r: Readlist, userId: string }) {
  ...
  return (
    <div
      className="rounded-2xl shadow-md flex flex-row gap-8 items-center w-full max-w-[1200px] font-poppins mb-8 cursor-pointer transition-colors"
      style={{
    ...
    >
      ...
      <div className="flex items-center" style={{ height: '100%' }}>
        <img
          ...
          style={{ width: 120, height: 120, minWidth: 120, maxWidth: 120, marginRight: 32 }}
        />
      </div>
      {/* Conteúdo principal do card */}
      <div className="flex-1 flex flex-col justify-center">
        <h2 className="text-h4 font-extrabold mb-2 text-[var(--secondary-700)]" style={{ marginBottom: 8 }}>{r.nome}</h2>
        ...
        <div className="text-b3 my-1 text-[var(--secondary-700)]">
          {Array.isArray(r.livros) ? r.livros.length : 0} livros
        </div>
        <div className="flex gap-2 mt-2">
          {/* Label pública/privada */}
          <span className="px-3 py-1 rounded text-b3 font-semibold" style={{ background: r.publica ? 'var(--info-100)' : 'var(--warning-100)', color: r.publica ? 'var(--info-700)' : 'var(--warning-700)' }}>
